...
function App() {
  const { state, loading, error, moveTask, removeTask, updateTask, addTask, clearZone, resetToDefault } = useTaskManager();
  const [activeTask, setActiveTask] = useState<Task | null>(null);
  const [currentView, setCurrentView] = useState<'today' | 'week' | 'projects' | 'focus'>('today');
    ...
    <div className="flex min-h-screen">
      ...
      <main className="flex-1 overflow-auto">
        <div className="max-w-[1100px] mx-auto p-6">
          ...
          {currentView === 'today' && loading && (
            <div className="text-center py-8">
              <p className="text-gray">Loading tasks from server...</p>
            </div>
          )}
          {currentView === 'today' && error && (
            <div className="bg-red-bg border border-red text-red px-4 py-3 rounded-lg mb-4">
          ...
          >
            <TaskZone
              id="yesterday"
              title="Yesterday's Activity (Feb 12)"
            ...
            <TaskZone
              id="today"
              title="Today's Priorities (Feb 13)"
              tasks={state.today}
